...
# braids trefoil 3 colors normal
if 1:
    reso = 40
    ...
    boundary_3D = np.array([[- 1, - 1, -np.pi * (1 + 0)],
                   [1, 1, np.pi * (1 + 0)]]) * scale
    fig = plot_cylinder(
        radius=0.93, height=2.05 * np.pi, fig=None, show=False
    )
    plot_ring(
        r_center=0, r_tube=0.93, z0=np.pi * 1.025, fig=fig, show=False, segments=30,
    )
    plot_ring(
        r_center=0, r_tube=0.93, z0=-np.pi * 1.025, fig=fig, show=False, segments=30,
    )
    ...
    pl.box_set_go(fig, mesh=None, autoDots=boundary_3D, perBox=0.01, aspects=[1.0, 1.0, 2], lines=False)
    fig.show()
    exit()
