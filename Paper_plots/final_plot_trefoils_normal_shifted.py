...
def plot_line_colored_dashed(dots, dots_bound=None, show=True, color=(0, 'black'), width=25, fig=None, save=None,
                      spheres=True):
    # Creating gaps to simulate dashes
    segment_length = 1
    gap_length = 2
    redscale = color
    x, y, z = dots[:, 0], dots[:, 1], dots[:, 2]
    ...
    for i in range(0, len(x) - segment_length + 1, total_length):
    # for i in range(5):

        # Remove the last None, if present
        x_dashed = x[i:i+segment_length]
        y_dashed = y[i:i+segment_length]
        z_dashed = z[i:i+segment_length]
...
dots3 = curve_3D(dots3[:, 0], dots3[:, 1], dots3[:, 2], resolution=200, smoothing_factor=70)
# print(dots1)
...
if 1:
    ...
    fig.update_layout(
        scene=dict(
            camera=dict(
                eye=dict(x=-0.3, y=-0.5, z=0.55),  # Adjust x, y, and z to set the default angle of view
                # up=dict(x=0, y=1, z=0)
            )
        )
    )
    ...
    fig.write_html(f'trefoil_torus_no_per_inverse.html')
    fig.show()
    exit()
...
# rotated + shifted
if 1:
    dots1rs = rotate_meshgrid(dots1[:, 0], dots1[:, 1], dots1[:, 2],
                              np.radians(0), np.radians(0), np.radians(60)+np.radians(40))
    dots1rs = np.array(dots1rs).T - np.array([0, 0, 0])  # - np.array([0, 0, 7.5])  # red
    dots2rs = rotate_meshgrid(dots2[:, 0], dots2[:, 1], dots2[:, 2],
                              np.radians(0), np.radians(0), np.radians(60))
    dots2rs = np.array(dots2rs).T - np.array([0, 0, 15])  # - np.array([0, 0, 15])  # blue
    dots3rs = rotate_meshgrid(dots3[:, 0], dots3[:, 1], dots3[:, 2],
                              np.radians(0), np.radians(0), np.radians(60))
    dots3rs = np.array(dots3rs).T + np.array([0, 0, 10])  # + np.array([0, 0, 15])  # green
    # fig = plot_cylinder(
    # 	radius=0.93, height=2 * np.pi, fig=None, show=False, dots_bound=boundary_3D
    # )
    scale = 1.2
    ...
    # pl.box_set_go(fig, mesh=None, autoDots=boundary_3D, perBox=0.01, aspects=[1.0, 1.0, 1.0], lines=False)
    color = ([0, '#660000'], [1, '#ff0000'])
    plot_line_colored(dots1rs, dots_bound=boundary_3D, show=False, color=color, width=width,
                      fig=fig, save=None, spheres=spheres)
    # color = ([0, '#007dff'], [1, '#000099'])
    color = ([0, '#000099'], [1, '#007dff'])
    plot_line_colored(dots2rs, dots_bound=boundary_3D, show=False, color=color, width=width,
                      fig=fig, save=None, spheres=spheres)
    # color = ([0, '#19ff19'], [1, '#134a0d'])
    color = ([0, '#134a0d'], [1, '#19ff19'])
    plot_line_colored(dots3rs, dots_bound=boundary_3D, show=False, color=color, width=width,
                      fig=fig, save=None, spheres=spheres)
    ...
    dots_normal = 1
    if dots_normal:
        dots1n = rotate_meshgrid(dots1[:, 0], dots1[:, 1], dots1[:, 2],
                                 np.radians(0), np.radians(0), np.radians(60))
        dots1n = np.array(dots1n).T
        dots2n = rotate_meshgrid(dots2[:, 0], dots2[:, 1], dots2[:, 2],
                                 np.radians(0), np.radians(0), np.radians(60))
        dots2n = np.array(dots2n).T
        dots3n = rotate_meshgrid(dots3[:, 0], dots3[:, 1], dots3[:, 2],
                                 np.radians(0), np.radians(0), np.radians(60))
        dots3n = np.array(dots3n).T
        color = ([0, '#660000'], [1, '#ff0000'])
        width_n = 8
        plot_line_colored_dashed(dots1n, dots_bound=boundary_3D, show=False, color=color, width=width_n,
                          fig=fig, save=None, spheres=spheres)
        # color = ([0, '#007dff'], [1, '#000099'])
        color = ([0, '#000099'], [1, '#007dff'])
        plot_line_colored_dashed(dots2n, dots_bound=boundary_3D, show=False, color=color, width=width_n,
                          fig=fig, save=None, spheres=spheres)
        # color = ([0, '#19ff19'], [1, '#134a0d'])
        color = ([0, '#134a0d'], [1, '#19ff19'])
        plot_line_colored_dashed(dots3n, dots_bound=boundary_3D, show=False, color=color, width=width_n,
                          fig=fig, save=None, spheres=spheres)
    ...
    fig.update_layout(scene=dict(camera=dict(projection=dict(type='orthographic'))))
    fig.write_html(f'trefoil_torus_rotated_shifted_schematic_no_perp_inverse.html')
    fig.show()
    exit()
