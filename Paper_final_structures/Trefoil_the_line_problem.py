...
# x, y = 2, 3
# ar = np.pi / 6
# x_p = (x * np.cos(ar) - y * np.sin(ar)) * 1
# y_p = (x * np.sin(ar) + y * np.cos(ar)) * 1
# x_ = (
...
y_scale2 = 1  # / 1.5 / np.sqrt(2) * 2  # * np.cos(np.pi/6) #/(1 + .5 * np.sin(np.pi/6))
x_scale3 = 1
y_scale3 = 1
...
def braid(x, y, z, angle=0, pow_cos=1, pow_sin=1, theta=0, a_cos=1, a_sin=1,
	...
	if 1:
		if braids_modification == 0:
			...
			betta = np.arctan(np.tan(ar) * x_scale2)
			phase_mask_fix = (phase > -np.pi) & (phase < -np.pi - betta)
			x_new[phase_mask_fix] *= 100
